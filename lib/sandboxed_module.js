...
SandboxedModule.prototype._createRecursiveRequireProxy = function() {
	var cache = {}
	var requried = this._getRequires()
	for (var key in requried) {
		var injectedFilename = requireLike(this.filename).resolve(key)
		cache[injectedFilename] = requried[key]
	}
	var globals = this.globals
	return function RecursiveRequireProxy(request){
		var requstedFilename = requireLike(this.filename).resolve(request)
		if (requstedFilename in cache) return cache[requstedFilename]
		var sandboxedModule = new SandboxedModule();
		sandboxedModule._getGlobals = function(){return globals}
		var trace = stackTrace.get(RecursiveRequireProxy)
		sandboxedModule._init(requstedFilename,trace,{
			locals:{
				require:RecursiveRequireProxy.bind(sandboxedModule),
			}
		});
		cache[requstedFilename] = sandboxedModule.exports
		return sandboxedModule.exports
	}
}
SandboxedModule.prototype._requireInterceptor = function() {
  var requireProxy = this._options.recursive? this._createRecursiveRequireProxy() :requireLike(this.filename, true);
  ...
  function requireInterceptor(request) {
    var exports = (inject && (request in inject))
      ? inject[request]
      : requireProxy.bind(self)(request);
