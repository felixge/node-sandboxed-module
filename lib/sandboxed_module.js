...
var instrumentMethod,
  ...
  coffee: function(source) {
    if (this.filename.search('.coffee$') !== -1){
      if(!coffeeMethod) {
        coffeeMethod = require('coffee-script').compile;
      }
      return coffeeMethod(source);
    } else {
      return source;
    }
  },
...
SandboxedModule.prototype._getCompileInfo = function() {
  ...
  var sourceToWrap = Object.keys(interceptors).reduce(function(source, name){
    return interceptors[name].bind(this)(source);
